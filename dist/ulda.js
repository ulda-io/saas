...
export const ulda0 = {
     ...
     */
    generateLinkedHashes: async (line) => {
     ...
     */
    first5Bytes: (signatureIteration, lineSize) => {
     ...
     */
    mergeSignatures: (array1, array2, extraByte) => {
     ...
     */
    backUpSignatureAssembler: async (line = null, height = 254) => {
     ...
     */
    backupOutPorter: async (base, iter) => {
    ...
    superAssembler: async (config) => {
        ...
        if (config.gap < 5) {
            ...
            const newSignatureForBackup = await ulda0.backupOutPorter(config.backUp, iteration);
        ...
        const newFile = await ulda0.newEncContentFile(config.jsonOrigin, config.password, newLinePorterWithBackup);
        // return part we say
        return {
            local: {
                // what stays here
     ...
     */
    signatureTypeCheck: async (signature) => {
        ...
        const signatureBuffer = signature instanceof Uint8Array
            ? signature.buffer
        ...
        const backupNormal = signature.slice(5 + requiredSignatureBytes, 5 + requiredSignatureBytes + 16);
        // Check for optional 33 bytes
        let backupToUpdate = null;
        let backupToUpdateNumber = null;
        const backupStatus = signature.length === 5 + requiredSignatureBytes + 16 + 33;
        if (backupStatus) {
            backupToUpdate = signature.slice(5 + requiredSignatureBytes + 16, 5 + requiredSignatureBytes + 16 + 32);
     ...
     */
    decSignatureTypeCheck: async (signature) => {
     ...
     */
    normalPackExporter: async (signature) => {
        ...
        if (signatureBuffer.byteLength !== expectedLength) {
            throw new Error('Incorrect buffer length, does not match the expected number of signatures and backup.');
        }
        const offset = 5; // Skip the first byte and the next 4 bytes
        const line = {};
        for (let i = 0; i < numSignatures; i++) {
            const start = offset + i * 24;
            line[signatureIteration + i] = new Uint8Array(signatureBuffer.slice(start, start + 24));
        }
        const backupOffset = 5 + numSignatures * 24;
        const backupNormal = new Uint8Array(signatureBuffer.slice(backupOffset, backupOffset + 16));
        let backupToUpdate = null;
        let backupToUpdateNumber = null;
        if (signatureBuffer.byteLength > backupOffset + 16) {
            const backupToUpdateStart = backupOffset + 16;
            backupToUpdate = new Uint8Array(signatureBuffer.slice(backupToUpdateStart, backupToUpdateStart + 32));
    ...
    fileDisassembler: async (config) => {
        ...
        const context = await ulda0.decContentFile(config.content, config.password, signatureLength);
        // voter part
     ...
     */
    objectsEqual: (obj1, obj2) => Object.keys(obj1).length === Object.keys(obj2).length &&
        Object.keys(obj1).every((key) => obj2[key] &&
            obj1[key].length === obj2[key].length &&
            obj1[key].every((v, i) => v === obj2[key][i])),
    /**
     ...
     */
    newEncContentFile: async (file, passwordSettings, signatureBytes) => {
        // encrypts file
        ...
        const key = await crypto.subtle.importKey('raw', passwordBytes, // 256 бит
     ...
     */
    decContentFile: async (encryptedData, passwordSettings, signatureLength) => {
        //decrypt any files
        const { password, iv, salt } = passwordSettings;
        ...
        const passwordBytes = encoder.encode(password).slice(0, 32);
        const key = await crypto.subtle.importKey('raw', passwordBytes, { name: 'AES-CBC' }, false, ['decrypt']);
        // Convert IV (string in hex) to Uint8Array.
        const ivArray = new Uint8Array((iv?.match(/.{1,2}/g) || []).map((byte) => parseInt(byte, 16)));
     ...
     */
    optimizePassword: async (passwordObject) => {
        ...
        if (!passwordObject.iv) {
            ...
            passwordObject.iv = Array.from(ivArray, (byte) => byte.toString(16).padStart(2, '0')).join('');
        }
        // Generate or use existing salt in Base64 format
        if (!passwordObject.salt) {
            const saltArray = crypto.getRandomValues(new Uint8Array(16));
            passwordObject.salt = btoa(String.fromCharCode.apply(null, Array.from(saltArray)));
        }
        ...
        const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(passwordObject.password), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
        // Getting PBKDF2 key
...
export class Ulda {
    constructor(apiKey, apiUrl = "https://api.0am.ch", dev) {
        ...
        Object.defineProperty(this, "masterFileData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "masterFileFullInfo", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "masterPassword", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "contentFiles", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        }); // crypted content File
        Object.defineProperty(this, "contentFileData", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        }); // encrypted content File
        Object.defineProperty(this, "contentFileFullInfo", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        }); // encrypted ContentData white full info
    ...
    async connect(masterFileIdOrName, password) {
        const { data, error } = await this.getMasterFile(typeof masterFileIdOrName === 'string'
    ...
    async createMasterFile(name, masterPassword) {
        ...
        return new Promise((resolve) => {
            ...
            this.socket.emit('master:init', { data: request, apiKey }, (response) => {
                this.connect(name, masterPassword);
                resolve(response);
            });
        });
    }
    async createContentFile(jsonOrigin, fileName) {
        if (this.contentFileData?.find((f) => typeof f.content !== 'string' && f.content.name === fileName))
            return { status: false };
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.socket.emit('content:create', {
                    data: response,
                    apiKey: this.apiKey,
                }, async (id) => {
                    ...
                    if (master) {
                        ...
                        if (master.password.password &&
                            ...
                            this.data[fileName] = {
                                ...
                                name: fileName,
                                content: { ...jsonOrigin, name: fileName },
                                update: async (updateFile) => {
                                    Object.assign(updateFile, {
                                        name: fileName,
                                    });
                                    const updateFileDto = mergeObjects(jsonOrigin.content, updateFile);
                                ...
                                delete: async () => {
                                    ...
                                    if (result?.status) {
                                        ...
                                        this.contentFiles = this.contentFiles.filter((file) => file.id === file.id);
                                        this.contentFileData = this.contentFileData.filter((file) => file.id === file.id);
                                        this.contentFileFullInfo =
                                            this.contentFileFullInfo.filter((file) => file.id === file.id);
                                        return { status: true };
                                    }
                                    return { status: false };
                                },
    ...
    async getMasterFile({ id, password, name, }) {
        return new Promise((resolve, reject) => {
            this.socket.emit('master:getOne', { apiKey: this.apiKey, id, name }, async (masterfileData) => {
                            ...
                            Object.assign(acc, {
                                [file.content.name]: {
                                    ...
                                    update: async (updateFile) => {
                                        ...
                                        if (typeof updateFile === 'object' &&
                                            updateFile !== null) {
                                            const { name, ...rest } = updateFile;
                                            updateFileWithoutName = rest;
                                        }
                                        const updateFileDto = mergeObjects(file.content, updateFileWithoutName);
                                        const result = await this.saveContentFile(file.id, updateFileDto);
                                        if (result?.data) {
    ...
    async updateMasterFile(jsonOrigin) {
        if (!this.masterFileFullInfo)
            return { error: 'Master file not found' };
        if (!this.masterFile?.id)
            return { error: 'Master file not found' };
        const packageConfig = {
            gap: this.storage.master[this.masterFile.id].iterationClient -
                    ...
                    const promises = result.map(async (file) => {
                        ...
                        return {
                            data: {
                                ...encrypted.data,
                                id: file.id,
                                name: encrypted.data?.jsonOrigin.name,
                            },
    ...
    async changeMasterFilePassword(newPassword) {
    ...
    async saveContentFile(id, content) {
        ...
        if (fileToUpdate &&
            ...
            const packageConfig = {
                gap: this.storage.content[fileToUpdate.jsonOrigin.name].iterationClient -
    ...
    async simulateBackUp(packageToSend, content, password, id) {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            data = await ulda0.superAssembler({
                gap: this.storage.content[id].iterationClient -
    ...
    async deleteContentFile(id) {
        ...
        if (fileToUpdate &&
            ...
            const packageToSend = await ulda0.superAssembler(packageConfig);
            return new Promise((resolve, reject) => {
                this.socket.emit('content:delete', {
                    contentFile: {
                        id,
                        signPack: packageToSend.global.signPack,
                        textBlock: packageToSend.global.textBlock,
                    ...
                    if (status)
                        ...
                        reject({ error: error || 'Error deleting file' });
                });
            });
        }
    }
    async deleteMasterFile() {
        if (!this.masterFileFullInfo)
            return { error: 'Master file not found' };
        if (!this.masterFile)
            return { error: 'Master file not found' };
        await Promise.all(this.contentFiles.map((file) => this.deleteContentFile(file.id)));
        const packageConfig = {
            gap: this.storage.master[this.masterFile.id].iterationClient -
                this.storage.master[this.masterFile.id].iterationServer,
            password: this.masterFileFullInfo.password,
        ...
        const packageToSend = await ulda0.superAssembler(packageConfig);
        return new Promise((resolve, reject) => {
            this.socket.emit('master:delete', {
                masterFile: {
                    id: this.masterFileFullInfo?.id,
                    name: this.masterFileFullInfo?.name,
                    key: this.masterFileFullInfo?.key,
    ...
    async connectToUpdateContentFile(updateFileByWebsocket) {
        const handleActionFunction = async (res) => {
            ...
            if (selectedMasterFile &&
                ...
                this.contentFiles = this.contentFiles.map((file) => file.id === res.id ? res : file);
                this.contentFileData = this.contentFileData.map((file) => file.id === res.id
                    ? {
                        ...data,
                        name: data.content.name,
                    }
                    : file);
                this.contentFileFullInfo = this.contentFileFullInfo.map((file) => file.id === res.id ? { ...file, ...result.data } : file);
    ...
    async connectToDeleteContentFile(updateFileByWebsocket) {
        const handleActionFunction = async (res) => {
            if ('status' in res && res.status) {
                return res;
            }
            ...
            if (selectedMasterFile &&
                ...
                this.contentFiles = this.contentFiles.filter((file) => file.id !== res.id);
                this.contentFileData = this.contentFileData.filter((file) => file.id !== res.id);
                this.contentFileFullInfo = this.contentFileFullInfo.filter((file) => file.id !== res.id);
    ...
    async connectToUpdateMasterFile(updateFileByWebsocket) {
        const handleActionFunction = async (res) => {
            if ('status' in res && res.status) {
                return res;
            }
            ...
            if (updateFileByWebsocket)
                return updateFileByWebsocket(masterFile);
        };
        this.socket?.on('master:update', handleActionFunction);
    }
    async disconnectSocket(event, handler) {
        SocketApi.instance?.off(event, handler);
    }
}
export function mergeObjects(prevObj, newObj) {
